...
const Role_Emojis = ["1⃣", "2⃣", "3⃣", "4⃣", "5⃣"];
const Ambulance_Embed = Global_Functions.Ambulance_Embed;

function Reaction_Role_Embed() {
    const Embed = new Discord.MessageEmbed().setTitle("Reaction Role").setDescription("A new reaction role has been created.").setColor(Global_Embed_Color);

    return Embed;
}

function Get_Message_ID(String) {
    let Match = String.match(/{messageid:.*}/gim);

    if (!Match) return null;

    let Replace_1 = Match[0].replace(/{messageid:\s?/gim, "");

    return Replace_1.replace(/}.*/gim, "");
}

function Get_Roles(String) {
    let Match = String.match(/{roles:.*}/gim);

    if (!Match) return null;

    let Replace_1 = Match[0].replace(/{roles:\s?/gim, "");
    let Replace_2 = Replace_1.replace(/}.*/gim, "");

    return Replace_2.split("}s?,s?{");
}

function Get_DM_User(String) {
    let Match = String.match(/{dmuser:.*}/gim);

    if (!Match) return null;

    let Replace_1 = Match[0].replace(/{dmuser:\s?/gim, "");
...
module.exports = {
    ...
    async execute(Message, Message_Args, Client, Command) {
        if (!Global_Databases.Moderation.Reaction_Role_Messages[Message.guild.id]) Global_Databases.Moderation.Reaction_Role_Messages[Message.guild.id] = {};

        const Message_ID = Get_Message_ID(Message.content);
        const Roles = Get_Roles(Message.content);
        let DM_User = Get_DM_User(Message.content);
        ...
        for (let Role = 0; Roles.length > Role; Role++) {
            Fetched_Message.react(Role_Emojis[Role]);
        }

        Message.channel.send({ embeds: [Reaction_Role_Embed()] });

        Global_Databases.Moderation.Reaction_Role_Messages[Message.guild.id][Message_ID] = {
