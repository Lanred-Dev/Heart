...
const Permanent_Commands = ["disablecommand", "disabledcommands"];
...
module.exports = {
    ...
    async execute(Message, Message_Args, Client) {
        const Requested_Command = Message_Args[0] ? Message_Args[0].toLowerCase() : null;
        ...
        Client.Commands.forEach((Command) => {
            if (Command.name === Requested_Command || Command.aliases.includes(Requested_Command)) {
                ...
                if (Global_Databases.Moderation[Message.guild.id].disabled_commands.length > 15 && Toggle === false)
                    return Message.channel.send({
                        embeds: [Ambulance_Embed(`${Message.author.toString()}, the max amount of disabled commands is 15.`)],
                    });
                if (Global_Databases.Moderation[Message.guild.id].disabled_commands.includes(Command.name) && Toggle === false)
                    return Message.channel.send({
                ...
                if (Toggle === false) {
                    const New_Array = Global_Databases.Moderation[Message.guild.id].disabled_commands.filter(function (Disabled_Command) {
                        return Disabled_Command != Requested_Command;
                    });

                    Global_Databases.Moderation[Message.guild.id].disabled_commands = New_Array;
                } else {
                    Global_Databases.Moderation[Message.guild.id].disabled_commands.push(Command.name);
                }
