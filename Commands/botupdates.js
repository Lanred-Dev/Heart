...
module.exports = {
    ...
    async execute(Message, Message_Args, Client) {
        ...
        if (Toggle === true) {
            ...
            if (Channel_Name === Global_Databases.Moderation[Message.guild.id].bot_updates.channel) return Message.channel.send({ embeds: [Ambulance_Embed(`${Message.author.toString()}, provide a new channel name.`)] });
            ...
            Channel.send({
                embeds: [new Discord.MessageEmbed().setTitle(":wrench: I am the new update channel :wrench:").setDescription("I am new the new update channel! All bot updates(and downtime messages) will be sent here.").setColor(Global_Embed_Color)],
            });
        }
        ...
        Message.channel.send({
            embeds: [
                new Discord.MessageEmbed()
                    .setTitle(":gear: [Gears turning] :gear:")
                    .setDescription(`Bot updates have been ${Toggle === true ? "enabled" : "disabled"} ${Toggle === true ? ` and the channel has been ${Global_Databases.Moderation[Message.guild.id].bot_updates.channel === null ? "set" : "updated"}` : ``}.`)
                    .setColor(Global_Embed_Color),
            ],
        });
        ...
        if (Log_Channel) {
            ...
            if (Toggle === true) {
                Log_Channel.send({
                    embeds: [Channel_Log_Embed(Channel, Global_Databases.Moderation[Message.guild.id].bot_updates.channel, Message.author.toString())],
                });
            }
        }

        Global_Databases.Moderation[Message.guild.id].bot_updates.enabled = Toggle;
        Global_Databases.Moderation[Message.guild.id].bot_updates.channel = Channel_Name;
    },
};
