...
function Dice_Embed(Roll_1, Roll_2, Status, User) {
    const Embed = new DiscordAPI.MessageEmbed()
        .setTitle("🎲 [Dice noises] 🎲")
        .setDescription(`:eyes: he rolles\n\n${User} Rolled\n***${Roll_1}***\n\nI Rolled\n***${Roll_2}***\n\n${Status}`)
...
module.exports = {
    ...
    async execute(Message, Message_Args, Client) {
        if (Moderation_Database.Currency_System[Message.guild.id].enabled === false) return Message.channel.send({embeds: [Currency_Disabled_Ambulance_Embed()]});
        ...
        if (Bet > Users_Database.Currency[Message.author.id].wallet && Betting != false) return Message.channel.send({embeds: [Ambulance_Embed(`You dont have **${Bet}**:coin: in your wallet to bet.`)]});
