...
function Leave_Embed(Message, Server, User_Tag) {
	const Embed = new Discord.MessageEmbed().setTitle(`:door: So long, ${User_Tag}. :door:`).setDescription(`${Message}`).setColor("#0091FF").setFooter(`❤ ${Server}`);
...
module.exports = async (Client, Member) => {
	...
	if (!Welcome_Database[Member.guild.id])
		(Welcome_Database[Member.guild.id] = {
			welcome: {
				role: null,
				channel: null,
				message: null,
			...
			fs.writeFileSync("Core/Databases/Welcome-Database.json", JSON.stringify(Welcome_Database, null, 2));
